...
class DynamoDBService:
    ...
    def update_to_table(self, table_name, myid, myname, myphone):
        # 通过dynamodb服务获取目标table的操作对象
        table_handle_h5_visit_info = self.get_service(table_name)

        '''update item'''
        response = table_handle_h5_visit_info.update_item(
            Key={
                'faceid': myid
            },
            UpdateExpression="set #ts = :s, #na = :n, #ph = :ph",
    ...
    def insert_to_passcode(self,table_name, myid,myttl,myotp):
        # 通过dynamodb服务获取目标table的操作对象
        table_handle_h5_visit_info = self.get_service(table_name)
        '''insert item'''
    ...
    def serach_table(self, table_name, key, value):
        # 通过dynamodb服务获取目标table的操作对象
        table_handle_h5_visit_info = self.get_service(table_name)
